...
int cng_signature_digest_sign_init(void *ctx, const char *mdname,
    ...
    cb_printf("SIGNATURE> cng_signature_digest_sign_init"); //xcs49 xcs52
    ...
    if (!ossl_digest_name_to_bcrypt_digest_name(sig_ctx, mdname, &sig_ctx->alg_identifier)) {
        cb_printf("SIGN> Unsupported hashing algorithm");
        return 0;
    }
...
int cng_signature_digest_sign_update(void *ctx,
                                     ...
                                     size_t datalen) {
    cb_printf("SIGNATURE> cng_signature_digest_sign_update");   //xcs54
    T_CNG_SIGNATURE_CTX *sig_ctx = (T_CNG_SIGNATURE_CTX *) ctx;
    ...
    if (p != NULL) {
        ...
        if (p->data_type == OSSL_PARAM_INTEGER) {
            if (*(int *) p->data == RSA_PKCS1_PSS_PADDING) {
                ...
                cb_printf("SIGN> Unknown padding mode requested as integer!");
                return 0;
            }
            ...
            if (!strcmp(p->data, OSSL_PKEY_RSA_PAD_MODE_PSS)) {
                ...
                cb_printf("SIGN> Unknown padding mode requested as string!");
                return 0;
            }
        } else {
            cb_printf("SIGN> Padding mode requested in unexpected data type!");
            return 0;
        }
        param_set = 1;
    }
    ...
    if (p != NULL) {
        if (p->data_type == OSSL_PARAM_INTEGER) {
            ...
            if (!strcmp(p->data, OSSL_PKEY_RSA_PSS_SALT_LEN_DIGEST)) {
                ...
                cb_printf("SIGN> Max salt length detection not implemented!");
                return 0;
            } else if (!strcmp(p->data, OSSL_PKEY_RSA_PSS_SALT_LEN_AUTO)) {
                cb_printf("SIGN> Automatic salt length detection not implemented!");
                return 0;
            } else {
                return 0;
            }
        } else {
            cb_printf("SIGN> Salt length received in unknown data type!");
            return 0;
        }
        param_set = 1;
    }
    if (!param_set) {
        cb_printf("SIGN> Setting unknown parameter!");
        return 0;
    }
    return 1;
}
